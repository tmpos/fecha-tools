...
export const FechaTools = (() => {
  ...
  const formatoHumano = (fecha = new Date(), idioma = "es", incluirHora = true) => {
    ...
    const hora = incluirHora ? `, ${f.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : "";
    const diaSemana = idioma === "es" ? ndiasemana[f.getDay()] : f.toLocaleString("en-US", { weekday: "long" });
    return idioma === "es"
  ...
  const rangoSemestre = (actual = true) => {
    ...
    if (actual) {
      ...
      if (semestre === 1) { año -= 1; inicioMes = 6; finMes = 11; }
      else { inicioMes = 0; finMes = 5; }
    }
  ...
  const nfecha = (pedido, opciones = {}) => {
    ...
    const rangos = {
      ...
      rangosemana: { fechainicio: `${inicioSemana.getFullYear()}-${pad(inicioSemana.getMonth() + 1)}-${pad(inicioSemana.getDate())} 00:00:00`, fechafin: fechaHoraFin },
      ...
      anioAnterior: { fechainicio: `${yyyy - 1}-01-01 00:00:00`, fechafin: `${yyyy - 1}-12-31 23:59:59` },
